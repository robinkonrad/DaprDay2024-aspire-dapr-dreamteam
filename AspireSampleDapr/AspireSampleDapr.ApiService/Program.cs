...
app.MapGet("/weatherforecast", async (DaprClient daprClient) =>
{
    ...
    if (cachedForecasts is not null && cachedForecasts.CachedAt > DateTimeOffset.UtcNow.AddSeconds(-10))
    {
    ...
    await daprClient.SaveStateAsync("statestore", "cache", new CachedWeatherForecast(forecast, DateTimeOffset.UtcNow));
